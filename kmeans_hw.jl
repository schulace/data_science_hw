...
# ╔═╡ 6e8ef5e0-90aa-11ef-3753-e7e19ef68aa0
begin
	using MLJ, DataFrames
	import RDatasets
end

# ╔═╡ 536b117d-ea66-473c-8f39-5cb15c42ee62
using PlutoUI

# ╔═╡ b61babf2-eba2-4ce2-9ac8-5a2c4e16dbb4
using Plots, StatsPlots

# ╔═╡ c2b7e1cd-f099-4813-8d14-0c3740fabce6
iris = RDatasets.dataset("datasets", "iris")

# ╔═╡ eb5cf49a-0373-4b69-a1da-a5bd927d94ea
describe(iris)

# ╔═╡ 97cb4f7e-3eea-4650-aec2-e9f758fc5183
X, y = select(iris, Not("Species")), select(iris, "Species")

# ╔═╡ 3efc2dba-0986-4a3d-8281-90a108a4643e
begin
	# normalize Xs
	μ = combine(X, All() .=> mean .=> All())
	σ = combine(X, All() .=> std .=> All())
	μ, σ
end

# ╔═╡ d2a80222-e38b-4219-91d8-90004ca738f3
X_norm = let
	(X .- μ) ./ σ
end

# ╔═╡ d75a34bd-d288-4ef3-a7b9-47a216abb685
begin
	import MLJClusteringInterface, Clustering
	KMeans = @load KMeans pkg=Clustering
end

# ╔═╡ 2fc31b30-236f-4815-af82-f08e1291ccca
@bind k Slider(2:1:6, show_value=true, default=3)

# ╔═╡ fa3ff160-519c-4d74-8d64-469a5baa5429
mach = let
	model = KMeans(k=k)
	mach = machine(model, X_norm)
	fit!(mach)
end

# ╔═╡ d658331c-5fb2-4877-a2e8-72f95f7c92da
model_centers = fitted_params(mach).centers

# ╔═╡ 2801b58f-fe1e-498f-8186-f405d5ed283f
function denorm(xs)
	(xs .* σ) .+ μ
end

# ╔═╡ fea25d72-ec9b-4173-bd12-94d9e2c60f7c
let
	p = plot()
	@df iris scatter!(p, :PetalLength, :SepalLength, group = :Species)
	centers = denorm(model_centers')
	@df centers scatter!(p, :PetalLength, :SepalLength, markersize=10.0, label="centers", title="Cluster centers on real labels")
end

# ╔═╡ e22de01d-be49-4675-b281-0e1e75198f7d
silhouette, benei_quality = let
	assignments = predict(mach, X_norm)
	sil = Clustering.clustering_quality(Matrix(X_norm)', model_centers, int(assignments), quality_index= :silhouettes)
	x_b = Clustering.clustering_quality(Matrix(X_norm)', model_centers, int(assignments), quality_index= :xie_beni)
	sil, x_b
end
...
manifest_format = "2.0"
project_hash = "50fe5baa97b160567087c4aedcede91704b2604b"
...
git-tree-sha1 = "c7efa910d7480eb25a61c701d9a9ef3e9a948afc"
