...
# ╔═╡ a41ec35d-7c82-4d21-a921-eef5758dce69
using Downloads, DataFrames, CSV, Plots, Statistics, StatsPlots, PlutoUI, MLJ, Tables

# ╔═╡ 40eab829-5100-491b-86af-fcfeb7478125
using MLJDecisionTreeInterface

# ╔═╡ 5188aa2b-4be4-43d6-bc3d-d528bddd59eb
import MLJBase

# ╔═╡ 1b531c86-e118-43a8-ac37-375ee309e713
raw_df = let
	file_loc = Downloads.download("https://archive.ics.uci.edu/ml/machine-learning-databases/wine-quality/winequality-red.csv")
	DataFrame(CSV.File(file_loc))
	
end

# ╔═╡ c75bc0cd-70f0-4ec2-a409-97f142f52488
propertynames(raw_df)

# ╔═╡ b30faa15-6db1-402b-abb7-3ea038d34b1f
describe(raw_df)

# ╔═╡ 70b6c4a1-4030-4ab8-8d49-0d3ed68e9bc4
# no missing data, as nmissing is 0

# ╔═╡ 616ab8d8-0ff5-40c3-a473-3a9704639f0b
X, y = raw_df[:, Not(:quality)], raw_df[:, :quality]

# ╔═╡ 6330f2ca-c223-4052-924c-2a2aa4736884
let
	x = raw_df[!,"alcohol"]
	y = raw_df[!,"quality"]
...
# ╔═╡ 66d9c045-7311-4f77-8145-411a843ac39b
@bind azimuth Slider(0:1:360, default=75, show_value=true)

# ╔═╡ 7eb961f5-dd8a-4efb-b140-488367c461b0
@bind elevation Slider(0:1:90, default=30, show_value=true)

# ╔═╡ fe47bf3e-6b06-4336-91e0-ccf0b978edc0

let
	x = raw_df[!, "pH"]
	y = raw_df[!, "fixed acidity"]
	z = raw_df[!, "quality"]
	plot(x, y, z, seriestype=:scatter, camera=(azimuth, elevation))
end

# ╔═╡ 9369b354-028f-4f36-9f0f-a9d910bdddb8
let
	cov_matrix = DataFrame(cov(Matrix(raw_df)), names(raw_df))
	cov_matrix = hcat(names(raw_df), cov_matrix)
	sort(cov_matrix, "quality")
end
# total S_O2 looks like the main determiner, with less meaning better wine

# ╔═╡ 157e2911-8532-492d-bcc4-77e2260671eb
X_norm = let
	function normalize(col)
		return (col .- mean(col)) ./ std(col)
	end
	cols_to_norm = filter(name -> eltype(X[!, name ]) <: Number, names(X))
	select(X, cols_to_norm .=> normalize .=> cols_to_norm, Not(cols_to_norm))
end

# ╔═╡ e6864294-a1af-45f6-a1dc-3f033b2b9ef2
DecisionTreeRegressor = @iload DecisionTreeRegressor pkg=DecisionTree

# ╔═╡ 7965354b-325a-4bc6-840c-3c1ba40cec7d
DecisionTreeClassifier = @iload DecisionTreeClassifier pkg=DecisionTree

# ╔═╡ 4bfc32d4-508d-4ed7-bd88-2120efbe4f07
((X_train, X_test), (y_train, y_test)) = let
	MLJ.partition((X_norm, coerce(y, OrderedFactor)), 0.85, shuffle=true, multi=true)
end

# ╔═╡ 28bb58eb-6dd8-44b7-85c3-5937940b95eb
# set up the model
default_mach = let
	model = DecisionTreeClassifier(rng=42069)
	mach = machine(model, X_train, y_train)
	fit!(mach)
end

# ╔═╡ fbf260a6-086c-4f1b-b8f1-cde734f69076
function acc_test_vs_train(machine)
	y_hat_train = predict_mode(machine, X_train)
	y_hat_test = predict_mode(machine, X_test)
	accuracy(y_hat_test, y_test), accuracy(y_hat_train, y_train)
end

# ╔═╡ c843de65-004c-4934-8b4a-810be3d421d9
# model evaluation, does perfect on training set, .65 on other, so overfitted
# will trim the tree down
acc_test_vs_train(default_mach), feature_importances(default_mach)

# ╔═╡ 2eb08db9-97e2-4974-ae09-8d0ff6901aa8
tuned_mach = let
	model = DecisionTreeClassifier(
		# avoid overfit
		max_depth=15,
		
		# going higher drops test accuracy
		min_samples_split=2, 

		# going higher severely drops test acc, I guess wines are unique enough
		# that forcing 2 to be on the same leaf misses some context
		min_samples_leaf=1,

		# forcing it to look at fewer subfeatures on each split helps
		n_subfeatures=6,

		# use gini like the default
...
# ╔═╡ 98f60f59-3353-4a70-9513-83eeb39ccc3c
acc_test_vs_train(tuned_mach), feature_importances(tuned_mach)

# ╔═╡ e934045b-fa9f-4d2e-a34f-0a43deffe6fa
begin
	# mostly gets confused by only 1 point in either direction, generally towards the mean
	y_hat = predict_mode(tuned_mach, X_test)
	confusion_matrix(y_test, y_hat)
	
end

# ╔═╡ 2eb8b467-dcbc-4565-8121-003428f8fd1d
accuracy(y_hat, y_test)

# ╔═╡ ada2f21f-3da6-45cb-803e-8a801788d1a6
l1(coerce(y_hat, Continuous), coerce(y_test, Continuous))

# ╔═╡ 7af0adb6-49b0-4518-8b64-3e2b12c72eef
l2(coerce(y_hat, Continuous), coerce(y_test, Continuous))
...
manifest_format = "2.0"
project_hash = "44877a409403de664249ec5ff3f5731634608dab"
