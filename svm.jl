...
# ╔═╡ 843b5e8e-85af-11ef-3392-2d522956b1bb
using DataFrames, Plots, MLJ, CSV

# ╔═╡ 4c7fdeb0-8066-4254-ab93-6a6100a73f4c
import Downloads

# ╔═╡ fa26e69a-5d43-410b-9671-72713c49a45a
raw_df = let
	file = Downloads.download("https://archive.ics.uci.edu/ml/machine-learning-databases/parkinsons/parkinsons.data")
	DataFrame(CSV.File(file))
end

# ╔═╡ aedc2a6c-3eec-4d36-8db6-4de483d66f62
X, y = raw_df[!, Not("name", "status")], raw_df[!, "status"]

# ╔═╡ 71e4a239-8be6-4657-a096-b68aab4324c7
# no missing values, all floats except our target
describe(raw_df)

# ╔═╡ 35bafe77-787f-4085-90a9-6375e103ec1a
function normalize(col)
	(col .- mean(col)) ./ std(col)
end

# ╔═╡ cbdf9743-0814-485e-98af-fb0b32dff073
X_norm = select(X, names(X) .=> normalize .=> names(X))

# ╔═╡ 15a852ea-c86a-4b77-bcec-e047b450d2a2
((X_train, X_test), (y_train, y_test)) = partition(
	(X_norm, coerce(y, Multiclass)),
	0.6,
	multi=true,
	shuffle=true)

# ╔═╡ da3967d8-98e1-4145-ae77-352998c49349
import MLJLIBSVMInterface

# ╔═╡ cd5d65a0-ef92-403f-a8ce-aad2601e63eb
import LIBSVM

# ╔═╡ 83b98c83-e7fe-49c9-b58e-0d91fa725e02
SVC = @iload SVC pkg=LIBSVM

# ╔═╡ 47d26fed-702e-4e16-9c90-834b5fb8f556
default_svm = let
	model = SVC(
...
# ╔═╡ 06a6259f-f99a-47a8-9533-e561ffec635a
y_hat_def = predict(default_svm, X_test)

# ╔═╡ 48df62d8-3419-481f-9a78-8813e7ad8ed6
begin
	# linear kernel is OK, but high false positive rate
	accuracy(y_hat_def, y_test),
	confusion_matrix(y_hat_def, y_test)
end

# ╔═╡ fcb2d829-73ef-4e9a-97ea-a21cb3032b34
tuned_svm = let
	model = SVC(
		# non-linear kernel as data doesn't seem linearly separable
...
# ╔═╡ 5d86844c-6f75-4e16-a5e5-2c9b579e0f58
y_hat_tuned = predict(tuned_svm, X_test)

# ╔═╡ 37200c27-1d79-428f-8461-2b3951ffcd26
# the best confusion matrix I could get
begin
	accuracy(y_hat_tuned, y_test),
	confusion_matrix(y_hat_tuned, y_test)
end
...
manifest_format = "2.0"
project_hash = "76edb330acd5d84cd894cc5127406181a8c46212"
