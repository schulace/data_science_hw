...
# ╔═╡ a4c91180-8679-11ef-3ec5-6788d54f4585
using DataFrames, CSV, Downloads, Statistics, MLJ

# ╔═╡ b6cad805-4ce3-461b-8726-de78279f033b
using PlutoUI

# ╔═╡ a15fa47e-6214-49c0-8127-df05f8215d3e
import DataFrames.transform

# ╔═╡ 4ff9c2e7-d3e0-48cc-aed2-6b1dfa876199
raw_df = let
	url = "https://archive.ics.uci.edu/ml/machine-learning-databases/wine-quality/winequality-red.csv"
	file_loc = Downloads.download(url)
	CSV.read(file_loc, DataFrame)
end

# ╔═╡ e0a00296-3936-4b25-9c64-d612b4e9dde9
describe(raw_df)

# ╔═╡ 8a5ed589-a14a-47e1-a054-fadca9bd89eb
function normalize(column)
	(column .- mean(column)) ./ std(column)
end

# ╔═╡ d44e51f3-db09-4b5f-9859-e86aefb3eb08
# normalize all floating point columns
norm_df = transform(
			raw_df,
			names(raw_df, Float64) .=> normalize .=> (names(raw_df, Float64)))

# ╔═╡ 9f49eba6-8058-48c4-8d88-eff5743fd1c3
combine(norm_df, All() .=> eltype ∘ scitype)

# ╔═╡ b34a8ab3-04f7-48c4-a796-655905341674
# we can see that everything is continuous except our count type
# which we want to be an OrderedFactor
let
	combine(norm_df, All() .=> eltype ∘ scitype)
	coerce!(norm_df, "quality" => Multiclass)
	norm_df
end

# ╔═╡ da699555-729c-46bc-aab8-17e1e099a4c5
combine(norm_df, All() .=> eltype ∘ scitype)

# ╔═╡ 87d278f7-487a-42b1-8307-680a255b781a
# split up the data into training and testing set
((X_train, X_test), (y_train, y_test)) = let
	X, y = norm_df[:, Not("quality")], norm_df[:, "quality"]
	partition((X, y), 0.85, shuffle=true, multi=true)
end

# ╔═╡ ba3065cc-52fc-4b15-8580-0656713cc397
begin
	import DecisionTree, MLJDecisionTreeInterface
	RandomForestClassifier = MLJ.@load RandomForestClassifier pkg=DecisionTree
end

# ╔═╡ 7e8505a3-f935-4adb-bd8a-1c0bf55e2174
import MLJBase.fit!

# ╔═╡ 74136ec4-a69d-4431-9c46-6f473f73ab4a

@bind hyperparams PlutoUI.combine() do Child
	md"""
	depth $(Child("depth", Slider(-1:20, default=17, show_value=true)))
	
	samps\_leaf $(Child("samps_leaf", Slider(1:10, default=4, show_value=true)))
	samps\_split $(Child("samps_split", Slider(2:20, default=10, show_value=true)))

	n\_subfeatures $(Child("n_subfeatures",
		Slider(
			1:length(names(X_train)),
			default=5,
			show_value=true)))
	samp\_fraction $(Child("samp_fraction",
		Slider(0.0:0.05:1.0, default=0.7, show_value=true)))

	n\_trees $(Child("n_trees", Slider(1:50, default=17, show_value=true)))
	"""
end

# ╔═╡ da296753-b166-450c-8ad4-47b5695bb55a
mach = let
	rf = RandomForestClassifier(
	max_depth=hyperparams.depth,
	min_samples_leaf=hyperparams.samps_leaf,
	min_samples_split=hyperparams.samps_split,
	n_subfeatures=hyperparams.n_subfeatures,
	n_trees=hyperparams.n_trees,
	sampling_fraction=hyperparams.samp_fraction,
	rng=42069
	)
	mach = machine(rf, X_train, y_train)
	fit!(mach, rows=1:length(y_train))
end

# ╔═╡ b2e17c5d-d708-453e-a526-c9a89017c250
let
	ŷ = predict_mode(mach, X_test)
	acc = accuracy(ŷ, y_test)
	l1_sc = l1(coerce(ŷ, Continuous), coerce(y_test, Continuous))
	l2_sc = l2(coerce(ŷ, Continuous), coerce(y_test, Continuous))
	cross_ent = cross_entropy(predict(mach, X_test), y_test)
	(;acc, l1_sc, l2_sc, cross_ent)
end

# ╔═╡ eb2b19c5-3fac-4bdf-9c9c-8c56632516ec
feature_importances(mach)

# ╔═╡ 0423c49f-601c-4d18-a437-1166bcf0f99e
# i used the sliders above to test various hyperparameters, and this was
# the best i could get. Allowing max depth, 7 samples per leaf, any numer
# of samples per split up to 15 was good, splitting on fewer features was
# better, and a sampling frequency of 70% of the data per tree worked
# nicely
...
manifest_format = "2.0"
project_hash = "906828f509d4857cf52409a18c7fec9df26fa059"
